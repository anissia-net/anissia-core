...
class EditCaptionCommand(
    ...
    val updDt: String = LocalDateTime.now().format(DTF_ISO_CAPTION),
    val website: String = ""
) {
    val updLdt: LocalDateTime get() =
        LocalDateTime.parse(updDt, DTF_ISO_CAPTION)
    ...
    fun validate() {
        ...
        require(isWebSite(website, true)) { "사이트주소는 공백이거나 http:// https:// 로시작해야합니다." }
        try {
            updLdt
        } catch (e: Exception) {
