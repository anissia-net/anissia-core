...
interface AnimeCaptionRepository : ReactiveCrudRepository<AnimeCaption, AnimeCaption.Key> {

    @EntityGraph(attributePaths = ["account"])
    @Query("SELECT a FROM AnimeCaption a WHERE a.anime.animeNo = :animeNo ORDER BY a.updDt DESC")
    fun findAllWithAccountByAnimeNoOrderByUpdDtDesc(animeNo: Long): Flux<AnimeCaption>
    ...
    fun findAllWithAnimeForAdminCaptionActiveList(an: Long, pageable: Pageable): Mono<Page<AnimeCaption>>

    @EntityGraph(attributePaths = ["anime"])
    @Query("SELECT a FROM AnimeCaption a JOIN a.anime b WHERE a.an = :an AND b.status = anissia.domain.anime.AnimeStatus.END ORDER BY a.updDt DESC")
    fun findAllWithAnimeForAdminCaptionEndList(an: Long, pageable: Pageable): Mono<Page<AnimeCaption>>

    @Modifying
    @Query("DELETE FROM AnimeCaption a WHERE a.anime.animeNo = :animeNo")
    fun deleteByAnimeNo(animeNo: Long): Mono<Int>
