            ...
            .flatMap {
                if (cmd.useCache) {
                    scheduleCacheStore.find(cmd.week) { getScheduleNotCache(cmd.week) }
                        .also { googleAnalyticsProxyService.send("/api/anime/schedule/${cmd.week}", exchange).subscribeBoundedElastic() }
                } else {
                    // 캐시 없는 호출은 관리자만 가능
...
${schedule.joinToString("\n") { """<tspan x="2" dy="20"><![CDATA[${it.time} ${it.subject}]]></tspan>""" }}
    ...
    private fun getScheduleNotCache(week: String): Mono<List<AnimeScheduleItem>> =
        animeRepository.findAllSchedule(week)
            .map { AnimeScheduleItem(it) }
            .collectSortedList { o1, o2 ->
                when(week) {
                    "7" -> o2.time.ifBlank { "9999" }.compareTo(o1.time.ifBlank { "9999" })
                    "8" -> o1.time.ifBlank { "9999" }.compareTo(o2.time.ifBlank { "9999" })
